...
Function do_ListDiskPartResult{       
    If($viewDataGrid.columncount -gt 0){  # ViewDataGrid sans colonne ?
        $viewDataGrid.DataSource = $null  # aucune source de données $null
        $viewDataGrid.Columns.Clear()     # efface les colonnes)
    }     
...
$pcInfo = New-Object System.Collections.ArrayList                           # idem pour récupérer les infos du PC
        # ----------------------------------------------
...
Remove-Item -Path $env:TEMP\ListDisk.txt 									  # efface l'ancien fichier
New-Item -Path $env:TEMP\ -Name ListDisk.txt -ItemType File -Force | Out-Null # création d'un nouveau fichier

$array = @(
  # pour chacun des disques, on affiche le détail pour récupérer chaque volume
  #
  for ($d=0;$d -le $totalDisk;$d++){                                       # récupère les informations sur les disks présent
    $diskID = $LISTDISK[-1-$d].Substring(8,2).trim()                       # numéro du disk (fr-FR index offset 8 en suprimant les espaces !)
    ...
    #write-host $detailDisk                                                # affiche l'intégralité du retour console
  
    #write-host $detailDisk[-1]                                            # permet d'accéder à la dernière ligne (retour console)
    #pause
  
    $IdxLigConsole=-1                                                      # Idx Nombre de volume détecté dans retour console
    $nbVolume=0                                                            # Nombre de volume détecté
    while ($detailDisk[$IdxLigConsole].Substring(2,6) -eq "Volume"){
      $IdxLigConsole=$IdxLigConsole-1                                      # on se place sur la ligne précédente
      $NbVolume=$NbVolume+1                                                # incrémente le nombre de volume détecté
    }
    
    #Write-Host "Le nombre de volume détecté est: $NbVolume"               # permet de contrôler le nombre de volume détecté
    #Pause

    For ($IdxVol=0; $IdxVol -lt $NbVolume; $IdxVol++){

      $name = $detailDisk[-18-$NbVolume].Trim()                            # 1Ere ligne de detail disk
      #write-host "nom du disque:$name"                                    # vrai si 1 volume, sinon c'est faux !!
    
      $driveLetter = $detailDisk[-1-$IdxVol].Substring(15,1)               # lettre du volume
      #write-host "Lettre du lecteur:$driveLetter"
   
      $type = $detailDisk[-16-$NbVolume].Substring(7).Trim()               # Type de disk ex SATA
      #write-host "Type de disque:$type"
  
      $size = $detailDisk[-1-$IdxVol].Substring(51,9).Trim()               # taille du disk
      #write-host "Size du disk:$size"
  
      $partitionType = $detailDisk[-1-$IdxVol].Substring(39,9).Trim()      # champ type partition
      #write-host "Type de partition:$partitionType"
  ...
  for($i=0;$i -lt $viewDataGrid.RowCount;$i++){                          # traitement de couleur sur les champs exploitables
    ...
    if($viewDataGrid.Item('DriveLetter',$i).Value.ToString() -like "C")
    {
      $viewDataGrid.Item('PartitionType',$i).Style.backcolor = 'red'     # support (media) non exploitable en rouge
    }
    else    
    {
      if($viewDataGrid.Item('PartitionType',$i).Value.ToString() -like "Partition")
      {
        $viewDataGrid.Item('PartitionType',$i).Style.backcolor = 'Orange'
      }
      if($viewDataGrid.Item('PartitionType',$i).Value.ToString() -like "Amovible")
      {
...
Function do_CopyFiles($diskLetter, $folder){                               
  ...
  $objFolder = $objShell.NameSpace($diskLetter)
    ...
    do_CreateImageContainer                                                # appel de fonction création structure dossiers
        ...
        [System.Windows.Forms.MessageBox]::Show("Le dossier n'existe pas !")  # boite de message "Le dossier n'existe pas !"
    }
}
