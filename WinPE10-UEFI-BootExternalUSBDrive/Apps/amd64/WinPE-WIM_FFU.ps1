...
$totalDisk = $diskID
If($totalDisk -lt 1){                                                    # si nombre de disque inférieur ou égal à 1...
    $mainImage = "X:\Windows\System32\PartitionPics\Warning.png"         # icône de mise en garde relatif Ramdisk (Winpe)
}
Else{
    $mainImage = "X:\Windows\System32\PartitionPics\AI.png"              # icône cube éclaté
}
...
# Vérification du type de BIOS / Firmware utilisé sur le PC
# MBR / UEFI ou encore inconnu
#
...
If($partitionWinCollection){
    ...
    foreach($partition in $partitionWinCollection){
            ...
            $labelPartitionDismToCapture.Text = "Création de l'image du lecteur $($partition.DeviceID) Partition"
    ...
    If($winPartitionFound -eq $false) {
                ...
                [System.Windows.Forms.MessageBox]::Show("Aucune partition windows trouvée!")
    }
 }
}

# Vérification des noeuds sélectionnés par l'utilisateur
#
...
# Déselectionne l'ensemble des noeuds
#
...
Function do_RemoveImageFiles($fileName){
        ...
        Else{
            ...
            [System.Windows.Forms.MessageBox]::Show("Fichier non retiré, S.V.P soyez sûre que le fichier existe")
        }
}
...
Function do_DeleteImageFiles {
    ...
    $confirmDelImgFile = [System.Windows.Forms.MessageBox]::Show("etes vous sûre de vouloir effacer les fichiers images sélectionnés?" , "Confirm Action!" , 4)
    If ($confirmDelImgFile -eq "YES") {
        ...
        [System.Windows.Forms.MessageBox]::Show("vous avez supprimé cette action.")
